...
function vehicleList() {
    $vehicle_list = array('' => Lang::get('general.select_vehicle')) + DB::table('vehicles')
    ->select(DB::raw('IF (vehicleno="" OR vehicleno IS NULL,name,concat(name, " / ",vehicleno)) as name, id'))
    ->orderBy('name', 'asc')
    ->whereNull('deleted_at')
    ->lists('name', 'id');
    return $vehicle_list;
...
function itemList() {
    $item_list = array('' => '') + DB::table('items')
    ->whereNull('deleted_at')
    ->orderBy('name', 'asc')
    ->lists('name', 'id');
    return $item_list;
...
function customersList() {
    $customer_list = array('' => Lang::get('general.select_customer')) + Supplier::orderBy('name', 'asc')
    ->orderBy('name', 'asc')
    ->lists('name', 'id');
...
function driversList() {
    $driver_list = array('' => Lang::get('general.select_driver')) + Supplier::orderBy('name', 'asc')
    ->orderBy('name', 'asc')
    ->lists('name', 'id');
    return $driver_list;
}
...
function itemTypeList() {
     $item_types= array('' => '','accessory' => 'Accessory', 'asset' => 'Asset', 'consumable' => 'Consumable');
     return $item_types;
}
