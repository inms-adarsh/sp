<?php
class Driver extends Elegant
{
	use SoftDeletingTrait;
	protected $dates = ['deleted_at'];
    protected $rules = array(
        'name'              => 'required|alpha_space|min:3|max:255|unique:drivers,name,{id}',
  ...
  /* public function assets()
    {
        return $this->hasMany('Asset', 'driver_id');
    }

    public function asset_maintenances()
    {
        return $this->hasMany('AssetMaintenance', 'driver_id');
    }

    public function num_assets()
    {
        return $this->hasMany('Asset', 'driver_id')->count();
    }

    public function licenses()
    {
        return $this->hasMany('License', 'driver_id');
    }

    public function num_licenses()
    {
        return $this->hasMany('License', 'driver_id')->count();
    }
    */
